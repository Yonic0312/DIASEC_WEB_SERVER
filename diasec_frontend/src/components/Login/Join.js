...
import axios from 'axios'; // Promise 기반으로 리액트에서 주로 쓰인다
...
const Join = () => {
    ...
    const checkEmailDuplicateSilent = async () => {
        ...
        try {
            const response = await axios.get(`${API}/member/check-email?email=${trimmedEmail}&provider=web`);
            if (response.data) {
    ...
    // 전화번호 중복 체크
    const checkPhoneDuplicateSilent = async () => {
        const phoneNumber = `${phone1}-${phone2}-${phone3}`;
    ...
    const handleRegister = async (e) => {
        ...
        const isPhoneAvailable = await checkPhoneDuplicateSilent();
        if (!isPhoneAvailable) return;

        const isEmailAvailable = await checkEmailDuplicateSilent();
        ...
        <div className="flex w-full">
            <form className="flex flex-col w-full" onSubmit={handleRegister}>
                ...
                <div className="flex flex-col">
                    ...
                    <span className="mt-3 ml-3 mb-3 sm:text-xs text-[11px]">[필수] 이용약관 동의</span>
                    <div className="ml-3 mr-3 mb-3 p-4 whitespace-pre-line overflow-y-scroll h-40 sm:text-sm text-[12px] border-[1px] text-gray-500">
                        {`제1조(목적)
                        이 약관은 디투원 운영하는 액자가게 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리?의무 및 책임사항을 규정함을 목적으로 합니다.

                        ※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」

                        제2조(정의)
                        ①“몰” 이란 디투원 회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
