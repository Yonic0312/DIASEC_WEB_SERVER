...
const Find_Id = () => {
    ...
    const changeContactType = (type) => {
        setContactType(type);
        if (type === 'email') setPhone('');
        ...
        <div className="flex items-center justify-center">
            <form onSubmit={handleFindId}>
                <div className="flex flex-col pt-20">
                    <span className="flex sm:text-xl text-[18px] font-bold sm:pb-2 justify-center">아이디 찾기</span>

                    <div className="flex mt-4 mb-2">
                        <div className="flex flex-col w-full gap-2">
                            <div className="flex items-center justify-center">
                                <div className='flex items-center sm:text-base text-[14px]' onClick={() => setContactType('email')}>
                            ...
                            <div className="
                                flex flex-col sm:w-full w-[226px] justify-between 
                                sm:text-[17px] text-[15px]">
                                <span>이름 </span> <input type="text" ref={nameRef} value={name} onChange={(e) => setName(e.target.value)} className="px-2 border-gray-300 border-[1px] w-full h-8"></input>
                            </div>
                            <div className="
                                flex flex-col sm:w-full w-[226px] justify-between 
                                sm:text-[17px] text-[15px]">
                                ...
                                {contactType === 'email' ? (
                                    <input 
                                        ...
                                        className="px-2 border-gray-300 border-[1px] w-full h-8" />
                                ) : (
                                    // 번호
                                    <input 
                                        type="text"
                            ...
                            <div className="flex w-full justify-center">
                                <button 
                                    ...
                                    disabled={loading}
                                    className={`
                                        w-full bg-black text-white p-4 
                    ...
                    <div className="flex flex-row gap-2 mb-3 justify-center">
                        <button
                            ...
                            className="
                                ...
                                sm:text-sm text-[12px]" 
                            onClick={() => navigate('/userLogin')}> 로그인 </button>
                        <button
                            type="button"
                            className="
                                border-[1px] w-1/2 h-[40px] 
                                sm:text-sm text-[12px]" 
                            onClick={() => navigate('/find_Pwd')}> 비밀번호 찾기 </button>
                    </div> 
                </div>
            </form>
        </div>
